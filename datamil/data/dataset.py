...
def apply_trajectory_transforms(
    ...
    *,
    ...
    training_idxs: Optional[Sequence[int]] = None,
    ...
    Args:
        ...
        task_augment_kwargs (dict, optional): Additional keyword arguments to pass to the task augmentation
            function.
        training_idxs (Sequence[int], optional): If provided, only trajectories with these indices will be
            included in the dataset. This is useful for creating a subset of a larger dataset.
    ...
    print('#'*10)
    print(sorted(training_idxs) if training_idxs is not None else None)
    print('#'*10)
    if training_idxs is not None:
        training_idxs = tf.convert_to_tensor(training_idxs, dtype=tf.int32)
        dataset = dataset.filter(
            lambda x: tf.reduce_any(x['index'][:, None] == training_idxs[None, :])
        )
...
def restructure(
    ...
    if index is not None:
        traj["index"] = index[:traj_len]
    # if sample_weights is not None:
    #     traj["weights"] = sample_weights[:traj_len]
    # else:
    #     traj["weights"] = tf.ones((traj_len,), dtype=tf.float32)
...
def make_dataset_from_rlds(
    ...
    **kwargs,
    ...
    if isinstance(dataset_statistics, str):
        ...
        ).traj_map(restructure_fn, num_parallel_calls)
    ...
    dataset = dataset.traj_map(restructure_fn, num_parallel_calls, deterministic=True)
    ...
    for dataset_kwargs, dataset_statistics, threads, reads in zip(
        ...
        dataset = apply_trajectory_transforms(
            ...
            **traj_transform_kwargs,
            num_parallel_calls=threads,
            train=train,
            training_idxs=dataset_kwargs.get('training_idxs', None)
